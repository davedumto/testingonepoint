...
export async function initiateAuth(userId: string, userEmail: string): Promise<string> {
  ...
  // Build authorize URL with state containing userId
...
export async function handleCallback(code: string, state: string) {
  ...
  if (tokenRes.ok && tokenData.access_token) {
    ...
    await OAuthEvent.create({
      ...
      authenticatedAt: new Date(),
      status: 'completed',
      metadata: { scope: tokenData.scope, tokenType: tokenData.token_type },
    });
