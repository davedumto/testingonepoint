      ...
      <div style={{ width: '100%', maxWidth: 420 }}>
        ...
        {sent ? (
          <div className="card" style={{ textAlign: 'center' }}>
            ...
            <p style={{ color: 'var(--muted)', fontSize: 14, lineHeight: 1.6 }}>If an account with <strong style={{ color: 'var(--ink)' }}>{email}</strong> exists, we sent a reset link. Expires in 15 minutes.</p>
            <Link href="/login" style={{ display: 'inline-block', marginTop: 24, fontSize: 14, fontWeight: 600, color: 'var(--teal)' }}>Back to sign in</Link>
          </div>
        ) : (
        ...
        <p style={{ textAlign: 'center', marginTop: 24, fontSize: 14, color: 'var(--muted)' }}>
          Remember your password?{' '}<Link href="/login" style={{ fontWeight: 600, color: 'var(--navy)' }}>Sign in</Link>
        </p>
      </div>
    </div>
  );
